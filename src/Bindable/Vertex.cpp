...
{
// VertexLayout
const VertexLayout::Element& VertexLayout::ResolveByIndex(
    size_t i) const
{
    return elements[i];
}
...
VertexLayout::Element::Element(ElementType type,
                               size_t offset)
...
constexpr size_t VertexLayout::Element::SizeOf(
    ElementType type)
{
    switch (type)
    {
...
ConstVertex::ConstVertex(const Vertex& v) : vertex(v) {}
...
Vertex VertexBuffer::Back()
{
    ...
    return Vertex{buffer.data() + buffer.size() -
                      layout.Size(),
                  layout};
}
...
{
    assert(i < Size());
    return Vertex{buffer.data() + layout.Size() * i,
                  layout};
}
