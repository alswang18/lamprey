...
DxgiInfoManager::DxgiInfoManager()
{
    ...
    typedef HRESULT(WINAPI * DXGIGetDebugInterface)(REFIID,
                                                    void**);

    // load the dll that contains the function
    // DXGIGetDebugInterface
    const auto hModDxgiDebug =
        LoadLibraryEx("dxgidebug.dll", nullptr,
    ...
    GFX_THROW_NOINFO(DxgiGetDebugInterface(
        __uuidof(IDXGIInfoQueue), &pDxgiInfoQueue));
}

void DxgiInfoManager::Set() noexcept
{
    // set the index (next) so that the next all to
    // GetMessages() will only get errors generated after
    // this call
    ...
    const
{
    ...
    for (auto i = next; i < end; i++)
    {
        ...
        auto bytes =
            std::make_unique<byte[]>(messageLength);
        auto pMessage =
            reinterpret_cast<DXGI_INFO_QUEUE_MESSAGE*>(
                bytes.get());
        // get the message and push its description into the
        // vector
        GFX_THROW_NOINFO(pDxgiInfoQueue->GetMessage(
