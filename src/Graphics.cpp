...
// graphics exception checking/throwing macros (some with
// dxgi infos)
...
#define GFX_THROW_INFO_ONLY(call)                          \
    ...
    {                                                      \
        ...
        if (!v.empty())                                    \
        {                                                  \
            throw Graphics::InfoException(__LINE__,        \
                                          __FILE__, v);    \
        }                                                  \
    }
#else
...
#define GFX_DEVICE_REMOVED_EXCEPT(hr)                      \
    Graphics::DeviceRemovedException(__LINE__, __FILE__,   \
                                     (hr))
...
Graphics::Graphics(HWND hWnd)
{
    ...
    GFX_THROW_INFO(D3D11CreateDeviceAndSwapChain(
        ...
        swapCreateFlags, nullptr, 0, D3D11_SDK_VERSION, &sd,
...
void Graphics::ClearBuffer(float red, float green,
                           float blue) noexcept
{
...
{
    // join all info messages with newlines into single
    // string
...
{
    return "Lamprey Graphics Exception";
}
...
std::string Graphics::HrException::GetErrorInfo()
    const noexcept
{
    return info;
}

const char* Graphics::DeviceRemovedException::GetType()
    const noexcept
{
    return "Lamprey Graphics Exception [Device Removed] "
...
void Graphics::DrawTestTriangle()
{
    ...
    // create vertex buffer (1 2d triangle at center of
    // screen)
    ...
    pContext->IASetVertexBuffers(
        0u, 1u, pVertexBuffer.GetAddressOf(), &stride,
        &offset);
    ...
    // bind render target
    pContext->OMSetRenderTargets(1u, pTarget.GetAddressOf(),
                                 nullptr);

    // Set primitive topology to triangle list (groups of 3
    // vertices)
...
const char* Graphics::InfoException::GetType()
    const noexcept
{
    return "Chili Graphics Info Exception";
}

std::string Graphics::InfoException::GetErrorInfo()
    const noexcept
{
    return info;
}