      ...
      dtheta(odist(rng)), dphi(odist(rng)), dchi(odist(rng))
{
    if (!IsStaticInitialized())
    {
        ...
        AddStaticBind(std::make_unique<
                      PixelConstantBuffer<ConstantBuffer2>>(
            gfx, cb2));
...
{
    return DirectX::XMMatrixRotationRollPitchYaw(pitch, yaw,
                                                 roll) *
           DirectX::XMMatrixTranslation(r, 0.0f, 0.0f) *
           DirectX::XMMatrixRotationRollPitchYaw(theta, phi,
                                                 chi) *
