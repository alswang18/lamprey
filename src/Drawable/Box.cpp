      ...
      dtheta(odist(rng)), dphi(odist(rng)), dchi(odist(rng))
{
    ...
    if (!IsStaticInitialized())
    {
        ...
        auto pvs = std::make_unique<VertexShader>(
            gfx, L"PhongVS.cso");
        ...
        AddStaticIndexBuffer(std::make_unique<IndexBuffer>(
            gfx, model.indices));
    ...
    else
    {
        ...
        colorConst.materialColor = material;
        AddBind(std::make_unique<
                PixelConstantBuffer<PSMaterialConstant>>(
            gfx, colorConst, 1u));
    }

    AddBind(std::make_unique<TransformCbuf>(gfx, *this));

    // model deformation transform (per instance, not stored
    // as bind)
    dx::XMStoreFloat3x3(
        &mt, dx::XMMatrixScaling(1.0f, 1.0f, bdist(rng)));
}
...
DirectX::XMMATRIX Box::GetTransformXM() const noexcept
{
    ...
    return dx::XMLoadFloat3x3(&mt) *
           dx::XMMatrixRotationRollPitchYaw(pitch, yaw,
                                            roll) *
