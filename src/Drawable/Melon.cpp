      ...
      dtheta(odist(rng)), dphi(odist(rng)), dchi(odist(rng))
{
    ...
    auto model = Sphere::MakeTesselated<Vertex>(
        latdist(rng), longdist(rng));
    // deform vertices of model by linear transformation
    model.Transform(dx::XMMatrixScaling(1.0f, 1.0f, 1.2f));

    AddBind(std::make_unique<VertexBuffer>(gfx,
                                           model.vertices));
...
{
    namespace dx = DirectX;
    return dx::XMMatrixRotationRollPitchYaw(pitch, yaw,
                                            roll) *
