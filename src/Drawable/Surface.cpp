...
{
    memset(pBuffer.get(), fillValue.dword,
           width * height * sizeof(Color));
}

void Surface::PutPixel(unsigned int x, unsigned int y,
                       Color c)
{
    assert(x >= 0);
    assert(y >= 0);
...
Surface::Color Surface::GetPixel(unsigned int x,
...
const Surface::Color* Surface::GetBufferPtrConst()
    const noexcept
{
    return pBuffer.get();
}
...
Surface Surface::FromFile(const std::string& name)
{
...

    {
        ...
        mbstowcs_s(nullptr, wideName, name.c_str(),
                   _TRUNCATE);
...
void Surface::Save(const std::string& filename) const
{
                    ...
                    CLSID* pClsid) -> void
    {
        ...
        if (pImageCodecInfo == nullptr)
        {
            std::stringstream ss;
            ss << "Saving surface to [" << filename
               << "]: failed to get encoder; failed to "
                  "allocate memory.";
            throw Exception(__LINE__, __FILE__, ss.str());
        }
        ...
        std::stringstream ss;
        ss << "Saving surface to [" << filename
           << "]: failed to get encoder; failed to find "
              "matching encoder.";
        throw Exception(__LINE__, __FILE__, ss.str());
    };
    ...
    // convert filenam to wide string (for Gdiplus)
    wchar_t wideName[512];
    mbstowcs_s(nullptr, wideName, filename.c_str(),
               _TRUNCATE);

    Gdiplus::Bitmap bitmap(
        width, height, width * sizeof(Color),
        PixelFormat32bppARGB, (BYTE*)pBuffer.get());
    if (bitmap.Save(wideName, &bmpID, nullptr) !=
        Gdiplus::Status::Ok)
    {
        std::stringstream ss;
        ss << "Saving surface to [" << filename
...
void Surface::Copy(const Surface& src)
{
    ...
    memcpy(pBuffer.get(), src.pBuffer.get(),
           width * height * sizeof(Color));
}
...
Surface::Exception::Exception(int line, const char* file,
                              std::string note) noexcept
    : LampreyException(line, file), note(std::move(note))
{
}
...
const char* Surface::Exception::what() const noexcept
{
    ...
    oss << LampreyException::what() << std::endl
        << "[Note] " << GetNote();
    whatBuffer = oss.str();
...
const std::string& Surface::Exception::GetNote()
    const noexcept
{
    return note;
}